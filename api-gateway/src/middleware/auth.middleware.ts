...
const verifyToken = async (token: string): Promise<boolean> => {
  try {
    ...
    if (typeof client.verifyToken === 'function') {
      const res = await client.verifyToken(String(token));
      return res?.data?.valid ?? res?.valid ?? true;
    }
    if (typeof client.verify === 'function') {
      const res = await client.verify(String(token));
      return res?.data?.valid ?? res?.valid ?? true;
    }
    return true;
  } catch {
...
const getCurrentUser = async (userId: string): Promise<{ emailVerified: boolean }> => {
  try {
    ...
    if (typeof client.getCurrentUser === 'function') {
      const res = await client.getCurrentUser(String(userId));
      return res?.data ?? res;
    }
    if (typeof client.getUser === 'function') {
      const res = await client.getUser(String(userId));
      return res?.data ?? res;
    }
...
const verifySession = async (sessionId: string, userId: string): Promise<boolean> => {
  try {
    ...
    if (typeof client.verifySession === 'function') {
      const res = await client.verifySession({ sessionId: String(sessionId), userId: String(userId) });
      return res?.data?.active ?? res?.active ?? true;
    }
    if (typeof client.checkSession === 'function') {
      const res = await client.checkSession({ sessionId: String(sessionId), userId: String(userId) });
      return res?.data?.active ?? res?.active ?? true;
    }
    return true;
  } catch {
...
const refreshToken = async (token: string): Promise<string | null> => {
  try {
    ...
    if (typeof client.refreshToken === 'function') {
      const res = await client.refreshToken({ token: String(token) });
