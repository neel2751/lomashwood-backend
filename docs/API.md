...
| Method | Path                        | Description                                                                                           |
| ------ | --------------------------- | ----------------------------------------------------------------------------------------------------- |
| `GET`  | `/api/v1/brochure`          | Get frontend brochure payload (`featuredBrochures`, `allBrochures`, `categories`, `requestBrochures`) |
| `POST` | `/api/v1/brochure/request`  | Submit brochure request form                                                                          |
| `POST` | `/api/v1/brochures/request` | Alias of the same brochure request endpoint                                                           |

Notes:

- `POST /api/v1/brochure/request` and `POST /api/v1/brochures/request` are equivalent and accept the same body.
- For `brochureIds`, frontend alias ids like `kitchen` and `bedroom` are supported and mapped server-side.
...
```bash
curl -X POST http://localhost:3000/api/v1/brochures/request \
