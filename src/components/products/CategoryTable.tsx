      ...
      <table className="w-full">
        ...
        <tbody className="divide-y divide-[#F0EEE9]">
          {isLoading ? (
            ...
            filtered.map((cat) => (
              <tr key={cat.id} className="group transition-colors hover:bg-[#FAF7F1]">
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={cn(
                        "flex h-8 w-8 items-center justify-center rounded-[8px] text-[11px] font-bold",
                        cat.type === "kitchen"
                ...
                <td className="px-5 py-4">
                  <span className="font-mono text-[12px] text-[#8A8884]">
                    /{cat.name.toLowerCase().replace(/\s+/g, "-")}
                  </span>
                </td>
                <td className="px-5 py-4">
                  <span className="line-clamp-1 max-w-[280px] text-[12.5px] text-[#6B6B68]">
                    {cat.type === "kitchen"
                      ? "All kitchen furniture, cabinets, and fitted units."
                      : "Fitted bedroom furniture, wardrobes, and storage."}
                  </span>
                ...
                <td className="px-5 py-4">
                  <Link
                    ...
                    className="inline-flex items-center gap-1 text-[12px] font-medium text-[#8B6914] transition-colors hover:text-[#6F5310]"
                  >
                    View
                    <ArrowRight size={13} />
                  </Link>
