...
export function UserDetail({ userId }: UserDetailProps) {
  ...
  const { data: user, isLoading, isError } = useUser(userId) as unknown as {
    data: User | undefined;
    isLoading: boolean;
    isError: boolean;
  };
  const { mutateAsync: deleteUser } = useDeleteUser() as unknown as {
    mutateAsync: (id: string) => Promise<null>;
  };
  const { mutateAsync: updateUser } = useUpdateUser() as unknown as {
    mutateAsync: (args: { id: string; payload: { status: string } }) => Promise<null>;
  };
  const { data: sessionsData } = useSessions({ userId, pageSize: 5 }) as unknown as {
    data: { data: Session[] } | undefined;
  };
  const recentSessions: Session[] = sessionsData?.data ?? [];
  ...
  const handleStatusUpdate = async (status: string) => {
    await updateUser({ id: userId, payload: { status } });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {user.status === "active" ? (
              <Button variant="outline" size="sm" onClick={() => handleStatusUpdate("suspended")}>
